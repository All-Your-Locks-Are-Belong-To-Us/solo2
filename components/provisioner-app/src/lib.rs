...
use apdu_dispatch::iso7816::{Instruction, Status};
use apdu_dispatch::{
    command::SIZE as CommandSize, response, response::SIZE as ResponseSize, Command,
};
use heapless::Vec;
...
use trussed::{
    ...
    syscall, Client as TrussedClient,
};
            ...
            Instruction::Unknown(ins) => {
                if let Ok(instruction) = Instructions::try_from(ins) {
                    ...
                    match instruction {
                        ...
                        WriteFile => {
                            if self.buffer_file_contents.len() == 0
                                || self.buffer_filename.len() == 0
                            {
                        ...
                        TestAttestation => {
                            ...
                            use trussed::{
                                types::KeyId, types::KeySerialization, types::Location,
                                types::Mechanism, types::Message, types::SignatureSerialization,
                            ...
                            match p1 {
                                _x if p1 == TestAttestationP1::P256Sign as u8 => {
                                    let sig: trussed::Bytes<MAX_SIGNATURE_LENGTH> =
                                ...
                                _x if p1 == TestAttestationP1::X255Agree as u8 => {
                                    ...
                                    let sig = syscall!(self
                                        .trussed
                                        .sign_hmacsha256(shared_secret, &challenge,))
                                    .signature;
    ...
    T: TrussedClient + client::X255 + client::HmacSha256,
{
    ...
    fn call(
        &mut self,
        _interface_type: apdu_dispatch::app::Interface,
        apdu: &Command,
        reply: &mut response::Data,
    ) -> apdu_dispatch::app::Result {
